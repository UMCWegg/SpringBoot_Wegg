...
public interface ContactFriendsRepository extends JpaRepository<ContactFriend, Long> {
    ...
    @Query("SELECT c FROM ContactFriend c WHERE c.user.id = :userId AND c.isFollowing = :status")
    List<ContactFriend> findByUserIdAndIsFollowingYet(@Param("userId") Long userId, @Param("status") FollowStatus status);
